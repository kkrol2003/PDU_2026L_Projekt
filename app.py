...
from utils import load_and_concat_data, get_unique_artists, train_RFC_model
...
rfc_model, feature_list = train_RFC_model(spotify_df)
...
with tab2:
    ...
    with st.sidebar:
        st.markdown("### 🎧 Wybierz gatunki do porównania:")
        ...
        st.markdown("### 🔍 Wybierz artystę:")
        st.markdown(
            "*Zostaw puste, aby analizować wszystkich artystów z wybranych gatunków*"
        )
    ...
    if not df_violin.empty:
        ...
        if len(selected_genres) > 0:
            ...
            st.warning(
                "Wybierz co najmniej jeden gatunek, aby zobaczyć wykres.", icon="⚠️"
            )
